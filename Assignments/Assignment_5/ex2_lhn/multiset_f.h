...
template<class T> void MultiSet<T>::display(void) // only for test purposes
{
    ...
    for (int i = 0; i < maxsize; ++i)
    {
        if (reprarray[i] == NULL)
            ...
            cout << "The entry " << i + 1 << " is " << (*(reprarray[i])).value << "   "<<"Multiplicity is " << (*(reprarray[i])).multiplicity << " .\n";
            }
    return;
}
    ...
    for (int i=0; i<maxsize; i++)
    {
        if(reprarray[i] != NULL && sets.reprarray[i] != NULL)
        {
            ...
            do{
                ...
                cout <<val<<"  "<<times<<"  "<<times2<<endl;

                if (times2 > times)
                {
                    int dif = times2-times;
                    for (int m=0; m<dif; m++)
                    {
                        insertion(val);
                    }
        ...
        ch_node<T>* temp2 = sets.reprarray[i];
        while (temp2 != NULL)
        {
            insertion(temp2->value);
            temp2 = temp2->pointer_next;
        }
    }
}
