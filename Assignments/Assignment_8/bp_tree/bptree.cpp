...
// function for assignment ex2

// Find the node that has the minimun key
int BPTree::find_min()
{
	Node* next = root;
	if (next == NULL)
...
// Find the node that has the maximum key
int BPTree::find_max()
{
	Node* next = root;
	if (next == NULL)
	...
	while ( !next->IS_LEAF )
	{
		next = next->ptr[next->size];
	}
	return next->key[next->size-1];
}

// Find the node's precessor
int BPTree::findPrec(int id)
{
	Node* x = search(id);
	int i = 0;
	int out;
	if (x == NULL)
	{
		cout << "ERROR: Input number is not found in the tree";
		return -1;
	}
	...
	cout<<"ERROR: The input number has no predecessor";
	return -1;
}

// Find the ndoe's successor
int BPTree::findSucc(int id)
{
	Node* x = search(id);
	int i = 0;
	int out;
	...
	while (x != root)
	{
		...
		// j is the index of the key in parent array
		while(j<parent->size)
		{
			if (parent->key[j] > id)
			{
	...
	cout<<"ERROR: The input number has no successor"<<endl;
	return -1;
}